...
def send_verification_email(username):
    ...
    verify_url = f"http://localhost:8090/verify/{token}"
...
def send_reset_email(username):
    ...
    reset_url = f"http://localhost:8090/reset/{username}/{token}"
...
def get_midterm():
    ...
    tpl = template("Midterm", items=items, message = "Logged in as " + username, status=None)
    save_session(response, session)
    return tpl
